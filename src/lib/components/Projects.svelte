...
<section id="projects" class="container-sm md:container-lg md:mx-auto px-5 max-w-6xl py-16">
	...
	{#each Object.entries(projects) as [cat, items], cat_idx}
		<div class="flex flex-col mb-1 {cat_idx % 2 ? 'md:flex-row-reverse' : 'md:flex-row'}">
			<div
				class="flex mb-2 md:basis-[24.5%] p-4 text-primary dark:text-primary-dark rounded-md border border-gray-300 shadow-md md:shadow-xl {cat_idx %
				2
					? 'md:ml-2'
					: 'md:mr-2'}"
			>
			...
			<article class="md:basis-3/4">
				...
				>
					{#each items as item}
						<article
							class="md:basis-[32.6%] mb-2 h-min text-primary dark:text-primary-dark rounded-md border border-gray-300 shadow-md md:shadow-xl {cat_idx %
							2
								? 'md:ml-2'
								: 'md:mr-2'}"
						>
